...
@AndroidEntryPoint
class FeedNoTravelJournalDialogFragment(
    private val onComplete: (Unit) -> Unit
) : BaseDialogFragment<DialogNoJournalBinding>(
    DialogNoJournalBinding::inflate,
) {
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        binding.btnDialogNoJournalCancel.setOnClickListener { dismiss() }
        binding.btnDialogNoJournalWrite.setOnClickListener {
            dismiss()
            onComplete(Unit)  }
    }
