...
@AndroidEntryPoint
class LifeShotSelectPlaceFragment :
    BaseMapFragment<FragmentLifeShotSelectPlaceBinding>(
        FragmentLifeShotSelectPlaceBinding::inflate,
        FragmentLifeShotSelectPlaceBinding::mapSelectPlace,
    ),
    OnMapReadyCallback {

    private val viewModel: LifeShotSelectPlaceViewModel by viewModels()
    ...
    private fun handleEvent(event: LifeShotSelectPlaceViewModel.Event) {
        when (event) {
            is LifeShotSelectPlaceViewModel.Event.SetMarker -> {
                map?.clear()
            ...
            is LifeShotSelectPlaceViewModel.Event.MoveMap -> {
                val cameraUpdate = CameraUpdateFactory.newLatLngZoom(event.latLng, DEFAULT_ZOOM)
                map?.moveCamera(cameraUpdate)
            }
            is LifeShotSelectPlaceViewModel.Event.OnComplete -> {
    ...
    private fun moveToPervPage(dto: SelectLifeShotPlaceDTO) {
        findNavController().previousBackStackEntry?.savedStateHandle?.set("placeName", dto.name)
        findNavController().popBackStack()
    }
