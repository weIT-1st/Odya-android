...
class FeedTravelJournalViewModel @AssistedInject constructor(
    ...
    @Assisted private val journalId: Long,
) : ViewModel() {

    @AssistedFactory
    interface FeedTravelJournalFactory {
        fun create(journalId: Long): FeedTravelJournalViewModel
    }

    private val _event = MutableEventFlow<FeedTravelJournalViewModel.Event>()
        ...
        getJob = viewModelScope.launch {
            ...
            if (result.isSuccess) {
                val newJournals = result.getOrThrow().map {
                    FeedTravelJournalEntity(it, journalId == it.travelJournalId)
                }
    ...
    fun onComplete(){
        viewModelScope.launch {
            _event.emit(Event.OnCompleted(selectedTravelJournal))
        }
    }
    ...
    sealed class Event {
        ...
        data class OnCompleted(val selectedTravelJournal : TravelJournalListInfo?) : Event()
    }

    companion object {
        fun provideFactory(
            assistedFactory: FeedTravelJournalViewModel.FeedTravelJournalFactory,
            journalId: Long,
        ): ViewModelProvider.Factory = object : ViewModelProvider.Factory {
            override fun <T : ViewModel> create(modelClass: Class<T>): T {
                return assistedFactory.create(journalId) as T
            }
        }
    }
}
